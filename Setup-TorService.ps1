...
function Exit-Script {
    ...
    Write-InstallLog "===== Setup-TorService.ps1 finished with exit code $Code ====="
...
Write-InstallLog "===== Setup-TorService.ps1 started ====="
...
if ($missing.Count -gt 0) {
    Write-InstallLog ("ERROR: Missing required file(s): " + ($missing -join ', '))
    Write-Host ""
...
if ($corrupt.Count -gt 0) {
    Write-InstallLog ("ERROR: File(s) present but 0 bytes (likely AV-quarantined): " + ($corrupt -join ', '))
    Write-Host ""
...
if (-not $isAdmin) {
    Write-InstallLog "ERROR: Not running as Administrator."
    Write-Host ""
...
Write-InstallLog "Attempting best-effort Defender exclusion for $AppDir ..."
try {
    Add-MpPreference -ExclusionPath $AppDir -ErrorAction Stop
    Write-InstallLog "Defender exclusion added (or already present)."
} catch {
...
if ($svc) {
    Write-InstallLog "Service $SvcName already exists - skipping install, will reapply config."
} else {
    Write-InstallLog "Installing service $SvcName via NSSM..."
    $out = & $Nssm install $SvcName $TorExe '-f' $Torrc 2>&1
    $out | Add-Content -Path $LogFile
    if ($LASTEXITCODE -ne 0) {
        Write-InstallLog "ERROR: nssm install failed with code $LASTEXITCODE"
        Write-Host "ERROR: nssm install failed. See $LogFile for details."
        Exit-Script -Code 1 -Silent:$Silent
    }
    Write-InstallLog "nssm install succeeded."
}
...
Write-InstallLog "Service configuration applied."

# ---- Firewall rule (idempotent) ----
$rule = Get-NetFirewallRule -DisplayName $FwRuleName -ErrorAction SilentlyContinue
if ($rule) {
    Write-InstallLog "Firewall rule already exists - skipping."
} else {
    Write-InstallLog "Adding firewall rule..."
    try {
        ...
        Write-InstallLog "Firewall rule added."
    } catch {
        Write-InstallLog "WARNING: failed to add firewall rule: $($_.Exception.Message)"
    }
}
...
Write-InstallLog "Registering TorAutoBridge-Watchdog scheduled task..."
try {
    ...
    Write-InstallLog "TorAutoBridge-Watchdog scheduled task registered (every 5 min + at boot, as SYSTEM)."
} catch {
...
if ($bootstrapped) {
    Write-InstallLog "SUCCESS: tor.exe is running and reported Bootstrapped 100%."
    Write-Host ""
    Write-Host "Tor service is installed, running, and connected."
    Exit-Script -Code 0 -Silent:$Silent
} elseif ($torAlive) {
    Write-InstallLog "WARNING: tor.exe process is alive but never reached Bootstrapped 100% within 15s - likely a bridge/network problem, not an install problem. Check $NoticeLog."
    Write-Host ""
    ...
    Write-InstallLog "ERROR: tor.exe never appeared as a running process. NSSM wrapper may be alive while tor.exe itself crashes/exits immediately (missing DLL, quarantined lyrebird.exe, bad torrc line)."
    Write-Host ""
